package com.flexproject.model
{
	...
	import com.flexproject.mock.MockConsumer;
	...
	public class FlexProjectPM implements IFlexProjectPM
	{
		...
		private var _consumer:MockConsumer;
		...
		public function messageEventHandler(msgEvt:MessageEvent):void
		{
			...
			countryVO.timeStamp = d;
			
			accumulateHits(countryVO);
		...
		private function accumulateHits(item:Object):void 
		{
			if (keys[item.code]!=null) {
				// Update the hits count for the item
				var storedItem:Object = keys[item.code];
				storedItem.hits = storedItem.hits+=1;
			} else {
				item.hits=1; 
				keys[item.code] = item;
